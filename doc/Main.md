...
what happened. The library has some extra bits for this as well.
...
`TestTree` is just a labeled rose tree -- it knows nothing about
WebDriver, assertions, or any of that.
...
1.  `test_suite` is a tree of individual `WebDriver` sessions. Each node
    in the tree is either an individual test (`TestCase`), a list of
    test trees (`TestGroup`), or a named test tree (`TestLabel`).
2.  `debugSuites` runs a list of test trees -- here there's only one.
    Note the reappearance of `defaultWebDriverConfig` and
    `defaultFirefoxCapabilities`; these are used for running the
    individual tests, just like in the previous examples. Those two
    `return ()` statements are placeholders for additional setup and
    teardown code, like auth or deleting saved screenshots, to run
    before and after the test trees.
3.  When run, `debugSuites` now uses the `TestLabel` info from our test
    trees to provide nested context on any failed assertions.
